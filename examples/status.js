const { status, statusFE01FA, statusFE01, statusFE } = require('minecraft-server-util');

// This will retrieve the status of any server with version 1.7 and up.
const statusExample = async () => {
	try {
		const result = await status('play.hypixel.net');
...
const statusFE01FAExample = async () => {
	try {
		const result = await statusFE01FA('play.hypixel.net');
...
const statusFE01Example = async () => {
	try {
		const result = await statusFE01('play.hypixel.net');
...
const statusFEExample = async () => {
	try {
		const result = await statusFE('play.hypixel.net');
